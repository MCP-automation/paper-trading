...
# ============================================================================
# STRATEGY 6: Momentum + Mean Reversion Scalp (MICRO-SCALP, HIGH FREQUENCY)
# ============================================================================

class Strategy6(StrategyBase):
    """
    Momentum + Mean Reversion Scalping Strategy - MICRO-SCALP, HIGH FREQUENCY
    
    Math-Based Entry Conditions (PRIMARY):
    
    MOMENTUM LONG:
    - (Close - Previous Close) / Previous Close > 0.001 (0.1% up move)
    
    MOMENTUM SHORT:
    - (Close - Previous Close) / Previous Close < -0.001 (0.1% down move)
    
    MEAN REVERSION FALLBACK (if momentum < 0.1%):
    - MR SHORT: Green candle (close > open) = expect pullback
    - MR LONG: Red candle (close < open) = expect bounce
    
    Risk Management (TIGHT for scalping):
    - Position Scale: 1.5 (increase 50%, tiny targets = tight SL)
    - Stop Loss: 0.15% (very tight)
    - Profit Targets: 60% @ 0.15%, 40% @ 0.30%
    - Expected R:R: 1.0:1 (scalp ratio, high win rate = edge)
    - Max 3 trades/day
    - Skip if spread > 0.05%
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Scalping = higher position size (tight SL = less notional)
        self.position_scale = config.get("position_scale", 1.5)
        self.max_trades_per_day = config.get("max_trades_per_day", 3)
    ...
    def generate_signal(self, df: pd.DataFrame, current_idx: int) -> Optional[str]:
        ...
        # Calculate momentum percentage (0.1% threshold)
        ...
        # FALLBACK: Mean reversion (opposite of candle color)
        if row['close'] > row['open']:  # Green candle → MR short
            return 'short'
        elif row['close'] < row['open']:  # Red candle → MR long
            return 'long'
    ...
    def get_signal_debug(self, df: pd.DataFrame, current_idx: int) -> Optional[Dict]:
        ...
        conditions = {
            'momentum_up': {'met': momentum_up, 'pct_change': round(pct_change * 100, 4), 'threshold': 0.1, 'msg': '+0.1% up move'},
            'momentum_down': {'met': momentum_down, 'pct_change': round(pct_change * 100, 4), 'threshold': -0.1, 'msg': '-0.1% down move'},
            'bullish_candle': {'met': bullish_candle, 'close': round(row['close'], 4), 'open': round(row['open'], 4)},
            'bearish_candle': {'met': bearish_candle, 'close': round(row['close'], 4), 'open': round(row['open'], 4)},
            'mr_short': {'met': mr_short, 'reason': 'Green candle → expect pullback'},
            'mr_long': {'met': mr_long, 'reason': 'Red candle → expect bounce'},
        }
        ...
        return {
            ...
            'risk_management': {
                'position_scale': self.position_scale,
                'stop_loss': '0.15%',
                'profit_targets': '60% @ 0.15%, 40% @ 0.30%',
                'expected_rr': 1.0,
                'max_trades_per_day': self.max_trades_per_day,
                'spread_limit': '0.05%',
                'note': 'Tight stops + high frequency = scalp edge'
            }
        }
